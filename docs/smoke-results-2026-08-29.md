...
- Observer received Target's `player_info` tab entry before vanish. After Target's `/vanish` acknowledgement and `alpha -> beta` move, Observer received Target's `player_info` add immediately followed by `player_remove`; unvanish later produced `player_info action=29` for Target on both clients. This proves observed tab-list masking/restoration packets. A later non-see entity probe directly observed Paper's Target entity destroy on vanish and entity spawn on unvanish; neither probe proves a no-flicker/one-Paper-tick cross-backend guarantee.
...
| Entity hiding | PASS at observed Paper-managed packet level in fixed follow-up; a non-see Observer received Target entity destroy on vanish and entity spawn on unvanish |
...
The fixed-artifact follow-up also ran `/tmp/vanish-nopacket-live-20260829/node-client/entity_transition_probe.js` with `see-uuids: false` and Observer removed from the Paper `ops.json` files, leaving only Target operator-level. Observer reached alpha and received Target's entity spawn at `14.570s` (`entityId=117`, type `156`); Target sent `/vanish` at `20.150s`, after which Observer received `entity_destroy entityId=117` at `20.219s`; Target toggled visible at `28.150s`, after which Observer received a matching entity spawn at `28.167s`. The probe completed with `clients=2 targetEntityIds={"Observer":117} events=16`. This directly covers local Paper API-only entity hide/show for a non-see viewer.

The remaining limitations are no-flicker timing, a one-Paper-tick cross-backend destination guarantee, and any zero-wire-packet claim. No FR-008 packet/NMS/ProtocolLib implementation was added.
